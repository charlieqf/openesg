...
const out=path.resolve(__dirname,'../verification/portfolio-20260910/online-resources.json');
...
(async()=>{
  ...
  const privatePaths=[];for(const route of ['/README.md','/wrangler.jsonc','/tests/runtime-baseline.json','/docs/portfolio-20260910.md']){const r=await fetch(base+route,{signal:AbortSignal.timeout(30000)});privatePaths.push({route,status:r.status});await r.arrayBuffer();}
